...
class ILI9341(object):
    ...
    def __init__(self, dc, rst, spi, width=ILI9341_TFTWIDTH,
    ...
    def reset(self):
        ...
        if self._rst is not None:
            self._rst.setValue(1)
            time.sleep(0.005)
            self._rst.setValue(0)
            time.sleep(0.02)
            self._rst.setValue(1)
            time.sleep(0.150)
...
dc = 17
dcObj = onionGpio.OnionGpio(dc)

reset = 16
resetObj = onionGpio.OnionGpio(reset)

resetObj.setOutputDirection(0)
dcObj.setOutputDirection(0)
...
# set the speed to 64MHz
spi.max_speed_hz=64000000
spi.mode = 0b00


disp = ILI9341(dcObj, resetObj, spi)
disp.begin()

#Clear Screen and Draw black rectangle
disp.set_window(0, 0)
dcObj.setValue(1)
for x in range(320*240):
	spi.writebytes([0x00, 0x00])
	
#Draw rectangle by pixel
for x in range(40):
	for y in range(40):		
		disp.set_window(x+100, y+140)
		dcObj.setValue(1)
